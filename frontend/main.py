...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            raw_data = root.data
            # When ADK packs an A2UI part, raw_data is often:
            # {"kind": "data", "metadata": {"mimeType": "application/json+a2ui"}, "data": {...}}
            if isinstance(raw_data, dict) and "surfaceUpdate" in raw_data:
                out.append({"kind": "a2ui", "data": raw_data})
            elif isinstance(raw_data, dict) and "data" in raw_data:
                meta = raw_data.get("metadata") or {}
                mime = meta.get("mimeType") if isinstance(meta, dict) else None
                inner_data = raw_data.get("data")
                if mime == _A2UI_MIME or (isinstance(inner_data, dict) and ("surfaceUpdate" in inner_data or "beginRendering" in inner_data)):
